...
def parse_tool_call(raw_text: str):
    """Parse a model-emitted tool call without using eval."""
    try:
    ...
    try:
        while '</answer>' not in response_message:
            ...
            if start_token in response_message:
                ...
                action_list = parse_tool_call(action_list)
                ...
                left    = max(0,        min(int(bbox[0] * width_scale ), ori_width  - 1))
                top     = max(0,        min(int(bbox[1] * height_scale), ori_height - 1))
                right   = max(left + 1, min(int(bbox[2] * width_scale ), ori_width     ))
                bottom  = max(top  + 1, min(int(bbox[3] * height_scale), ori_height    ))
...
if __name__ == "__main__":
    print(f"IMAGE_FACTOR: {IMAGE_FACTOR}, MIN_PIXELS: {MIN_PIXELS}, MAX_PIXELS: {MAX_PIXELS}.")
