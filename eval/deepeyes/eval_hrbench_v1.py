...
def run_zoom_loop_for_region(
    ...
    k, iou_thr, max_iter, max_level, expand_mode,
):
    """
    ...
    for iter_idx in range(1, max_iter+1):
        ...
        for level_idx in range(1, max_level+1):
            ...
            new_w, new_h = smart_resize(i_r - i_l, i_b - i_t, max_pixels=MAX_PIXELS)
            ...
            messages = [
                ...
                {"role": "user", "content": [
                    {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{base64_img}"}, "max_pixels": MAX_PIXELS},
                    {"type": "text", "text": q_opts + USER_PROMPT_CUSTOM.format(idx=level_idx)},
                ]},
            ]
            try:
                params = {
        ...
        if len(level_data) <= 1:
            ...
            all_A = [ld["A_bbox"] for ld in level_data]
            A0_bbox = bbox_min_envelope_multi(all_A)
        ...
        converged = True
        min_level = 0 if max_level == 1 else 1
        for lev in range(min_level, max_level):
            if ious[lev] is None or ious[lev] <= iou_thr:
                converged = False
                break
        if converged:
            break

    return (A0_bbox, zoom_details)
...
def process(idx_arg):
    ...
    try:
        ...
        for zoom_idx, (bbox_orig, label) in enumerate(screened_regions):
            ...
            A_final, zoom_details = run_zoom_loop_for_region(
                pil_img, img_w, img_h, bbox_orig, label, question, option_str,
                args.k, args.iou_thr, args.max_iter, args.max_level, args.expand_mode,
            )
            final_regions.append((A_final, label))
            all_zoom_details.append({"label": label, "zoom_details": zoom_details})

            if zoom_details:
                last_iter = zoom_details[-1]
                for t in last_iter.get("think_blocks", []):
                    if t.strip():
                        think_blocks_for_step10.append(t)

            for iter_idx, zd in enumerate(zoom_details):
                for level_idx, ld in enumerate(zd.get("level_data", [])):
                    print_messages.append({
                        "role": "assistant",
                        "content": ld.get("response", ""),
                        "meta": "step7_zoom{}_iter{}_level{}".format(zoom_idx + 1, iter_idx + 1, level_idx + 1),
                    })
