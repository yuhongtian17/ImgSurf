...
# SGLang 0.5.6 moved the engine request dataclasses out of
# tokenizer_manager. Import them from their canonical public location so the
# rollout module does not depend on the pre-0.5.6 re-exports.
...
# SGLang's normal initializer registers a process signal handler. Ray invokes
# this engine from an async actor thread, where signal.signal raises ValueError
# (https://github.com/sgl-project/sglang/issues/6723). Keep the 0.5.6
# environment/version setup, but intentionally omit that signal registration.
def _set_envs_and_config(server_args: ServerArgs):
    ...
    os.environ.setdefault("CUTE_DSL_LOG_LEVEL", "30")
    os.environ.setdefault("CUTE_DSL_LOG_TO_CONSOLE", "1")
    os.environ["SGLANG_RUN_ID"] = f"sglang-run-{time.time()}-{uuid4().hex}"
...
class SGLangRollout(BaseRollout):
    ...
    def _init_inference_engine(self, trust_remote_code, actor_module, port):
        ...
        if nnodes > 1:
            # get_ip() was removed in SGLang 0.5.6. The replacement also
            # honors SGLANG_HOST_IP/HOST_IP for multi-node deployments.
    ...
    async def _handle_engine_generate(
        ...
        # Multi-turn agent loops may reserve part of the rollout budget for a
        # later answer/refinement turn.  Respect their per-request cap instead
        # of replacing it with the global response length.
        requested_max_new_tokens = int(sampling_params.get("max_new_tokens", self.config.response_length))
        max_new_tokens = min(
